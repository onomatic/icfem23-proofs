...
lemma lengthS_zip: "lengthS (zip_trace f s s') = 
  ...
  apply (rule lengthS_eqI)
   apply (clarsimp simp: index_zip_iff split: if_splits)
   apply (metis finiteS_def lengthS_def option.distinct(1))
   apply (clarsimp simp: index_zip_iff split: if_splits)
    apply (metis LeastI finiteS_def isl_monotone leI lengthS_def not_less_Least option.distinct(1) option.sel)
  apply (metis LeastI finiteS_def isl_monotone leI lengthS_def not_less_Least option.distinct(1) option.sel)
  apply (metis LeastI finiteS_def isl_monotone leI lengthS_def not_less_Least option.distinct(1) option.sel)
  done
...
lemma map_zip: "map_trace g (zip_trace f t t') = zip_trace (\<lambda>x y. g (f x y)) t t'"
  ...
  apply (clarsimp simp: index_map_sum)
  apply (case_tac "index (zip_trace f t t') n"; clarsimp)
...
lemma zip_map_r: "zip_trace f t (map_trace g t') = zip_trace (\<lambda>x y. f x (g y)) t t'" 
  by (rule trace_eqI, clarsimp simp: index_zip_iff index_map_sum, case_tac "index t n"; case_tac "index t' n"; clarsimp)


definition "repeatN n s = liftF (\<lambda>m. if m < n then Inl s else Inr Term)"
...
lemma index_zip_Inl_iff: "index (zip_trace f t t') n = Inl x \<longleftrightarrow> (\<exists>a b. index t n = Inl a \<and> index t' n = Inl b \<and> x = f a b)"
  apply (safe)
   apply (clarsimp simp: index_zip_iff split: if_splits, case_tac "index t n"; case_tac "index t' n"; clarsimp)
  by (clarsimp simp: index_zip_iff split: if_splits)
...
lemma match_inf: "lengthS x = None \<Longrightarrow> match x x'" 
  apply (clarsimp simp: match_def finiteS_def)
  by (metis finiteS_def lengthS_def option.distinct(1))

lemma index_take: "index (takeS n t) m =  (if m < n then (index t m) else Inr Term)"
  apply (subst takeS_def, subst index_liftF)
   apply (clarsimp simp: traceDom_def)
   apply (metis dual_order.refl index_eq_finished sum.collapse(2) symbols.exhaust)
  apply (clarsimp)
  done
...
lemma lengthS_takeS_min': "lengthS x = None \<Longrightarrow> lengthS (takeS y x) = Some (y)"
  by (metis (no_types, lifting) finiteS_def finiteS_takeS 
  index_take isl_iff leI lengthS_def nle_le option.distinct(1) order_less_irrefl)
...
lemma last_map[simp]: "StateTrace.last (map_trace f t) = map_option f (last t)"
...
lemma glue_l1_trace: "lengthS y = Some 1 \<Longrightarrow> match x y \<Longrightarrow> (x -o y) = x"
  apply (rule trace_eqI)
  apply (case_tac "finiteS x"; clarsimp?)
  apply (clarsimp simp: finite_index_iff index_tail_iff)
   apply (smt (verit, del_insts) Suc_less_eq isl_iff lengthS_def less_nat_zero_code 
       option.distinct(1) option.exhaust_sel sum.expand symbols.exhaust)
  by (simp add: infinite_index_iff)
